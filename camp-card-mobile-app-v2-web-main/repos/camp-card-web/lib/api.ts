...
export const api = {
  ...
  // ============ DEVICES ============
  getDevices: async (session?: Session | null) => {
    try {
      const result = await apiCall<any>('/devices', {}, session);
      return {
        content: result.content || result.devices || result || [],
      };
    } catch (error) {
  ...
  getDeviceById: async (id: string, session?: Session | null) => {
    try {
      return await apiCall<any>(`/devices/${id}`, {}, session);
    } catch (error) {
  ...
  createDevice: async (data: any, session?: Session | null) => {
    try {
      return await apiCall<any>('/devices', {
        method: 'POST',
        body: JSON.stringify(data),
      }, session);
    } catch (error) {
  ...
  updateDevice: async (id: string, data: any, session?: Session | null) => {
    try {
      return await apiCall<any>(`/devices/${id}`, {
        method: 'PUT',
        body: JSON.stringify(data),
      }, session);
    } catch (error) {
  ...
  deleteDevice: async (id: string, session?: Session | null) => {
    try {
      await apiCall<any>(`/devices/${id}`, {
        method: 'DELETE',
      }, session);
      return { success: true };
    } catch (error) {
  ...
  updateDeviceStatus: async (id: string, status: string, session?: Session | null) => {
    try {
      return await apiCall<any>(`/devices/${id}/status`, {
        method: 'PATCH',
        body: JSON.stringify({ status }),
      }, session);
    } catch (error) {
  ...
  getDevicesByMerchant: async (merchantId: string, session?: Session | null) => {
    try {
      const result = await apiCall<any>(`/devices/merchant/${merchantId}`, {}, session);
      return {
        content: result.content || result.devices || result || [],
      };
    } catch (error) {
      console.error('Failed to fetch merchant devices:', error);
      return { content: [] };
    }
  },

  getDeviceCatalog: async (session?: Session | null) => {
    try {
      return await apiCall<any>('/devices/catalog', {}, session);
    } catch (error) {
  ...
  getDeviceStats: async (session?: Session | null) => {
    try {
      return await apiCall<any>('/devices/stats', {}, session);
    } catch (error) {
  ...
  assignDeviceToMerchant: async (deviceId: string, merchantId: string, locationId?: string, session?: Session | null) => {
    try {
      const url = locationId
        ? `/devices/${deviceId}/assign?merchantId=${merchantId}&locationId=${locationId}`
        : `/devices/${deviceId}/assign?merchantId=${merchantId}`;
      return await apiCall<any>(url, {
        method: 'POST',
      }, session);
    } catch (error) {
      console.error('Failed to assign device to merchant:', error);
      throw error;
    }
  },

  unassignDevice: async (deviceId: string, session?: Session | null) => {
    try {
      return await apiCall<any>(`/devices/${deviceId}/unassign`, {
        method: 'POST',
      }, session);
    } catch (error) {
  ...
  completeDeviceOnboarding: async (deviceId: string, session?: Session | null) => {
    try {
      return await apiCall<any>(`/devices/${deviceId}/complete-onboarding`, {
        method: 'POST',
      }, session);
    } catch (error) {
