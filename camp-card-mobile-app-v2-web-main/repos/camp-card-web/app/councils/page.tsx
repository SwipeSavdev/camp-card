...
function Icon({ name, size = 18, color = 'currentColor' }: { name: string; size?: number; color?: string }) {
  const icons: Record<string, JSX.Element> = {
    ...
    building: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2">
...
interface TroopLeaderCandidate {
  id: string;
  name?: string;
  firstName?: string;
  lastName?: string;
...
export default function CouncilsPage() {
  ...
  const [_newLeaderName, setNewLeaderName] = useState('');
  const [_newLeaderRole, setNewLeaderRole] = useState('');
  const [_selectedCouncilForTroop, _setSelectedCouncilForTroop] = useState('');
  const [_selectedTroopForLeader, _setSelectedTroopForLeader] = useState('');
  ...
  return (
    <PageLayout title="Councils & Troops" currentPath="/councils">
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: '13px', color: themeColors.gray600 }}>
            ...
            {startIndex + 1}
            -
            {Math.min(endIndex, filteredCouncils.length)}
            {' '}
            of
        ...
        >
          {/* Search Input */}
          <div style={{
            position: 'relative', flex: '1', minWidth: '200px', maxWidth: '400px',
          }}
          >
            <div style={{
      ...
      {showAddCouncil && (
      <div style={{
        backgroundColor: themeColors.white, borderRadius: themeRadius.lg, padding: themeSpace.xl, marginBottom: themeSpace.xl, boxShadow: themeShadow.md, border: `1px solid ${themeColors.primary200}`,
      }}
      >
        <h3 style={{
          margin: `0 0 ${themeSpace.lg} 0`, color: themeColors.text, fontSize: '16px', fontWeight: '600',
        }}
        >
          Create New Council
        </h3>
      ...
      {filteredCouncils.length === 0 ? (
        <div style={{
          textAlign: 'center', padding: themeSpace.xl, backgroundColor: themeColors.white, borderRadius: themeRadius.lg, color: themeColors.gray600,
        }}
        >
        ...
        <div style={{ display: 'grid', gap: themeSpace.lg }}>
          {paginatedCouncils.map((council) => (
            <div
              key={council.id}
              style={{
                backgroundColor: themeColors.white, borderRadius: themeRadius.lg, overflow: 'hidden', boxShadow: themeShadow.sm, border: `1px solid ${themeColors.gray200}`,
              }}
            >
              {/* Council Header */}
              <div
              ...
              >
                ...
                >
                 ...
                 >
                   ...
                   {council.location}
                   {' '}
                   {council.troopUnits.length}
                   {' '}
                   troop unit
                 {council.troopUnits.length !== 1 ? 's' : ''}
                 </p>
                 </div>
                </div>
                <button
              ...
              <div style={{ padding: themeSpace.lg, backgroundColor: themeColors.gray50, borderTop: `1px solid ${themeColors.gray200}` }}>
               ...
               <div style={{
               backgroundColor: themeColors.white, borderRadius: themeRadius.card, padding: themeSpace.lg, marginBottom: themeSpace.lg, border: `1px solid ${themeColors.info50}`,
             }}
             >
               <h4 style={{
               margin: `0 0 ${themeSpace.lg} 0`, fontSize: '14px', fontWeight: '600', color: themeColors.text,
             }}
             >
               New Troop Unit
             </h4>
         ...
         Cancel
                ...
                {council.troopUnits.length === 0 ? (
                 <p style={{
                 color: themeColors.gray500, fontSize: '13px', margin: 0, padding: themeSpace.lg, textAlign: 'center',
               }}
               >
               No troop units yet. Add one to get started.
               </p>
               ) : (
               <div style={{ display: 'grid', gap: themeSpace.lg }}>
               {council.troopUnits.map((troop) => (
                 ...
                 key={troop.id}
                 style={{
                 backgroundColor: themeColors.white, borderRadius: themeRadius.card, overflow: 'hidden', border: `1px solid ${themeColors.gray200}`,
               }}
               >
                 {/* Troop Header */}
                 <div
               ...
               >
                 <div style={{
                 display: 'flex', alignItems: 'center', gap: themeSpace.md, flex: 1,
               }}
               >
                 ...
                 <div>
                 <h4 style={{
                   margin: 0, color: themeColors.text, fontWeight: '600', fontSize: '15px',
                 }}
                 >
                   {troop.name}
                 </h4>
                 <p style={{ margin: `${themeSpace.xs} 0 0 0`, fontSize: '12px', color: themeColors.gray600 }}>
                   Leader:
{troop.leaderName}
                 </p>
               </div>
             ...
             <div style={{
               backgroundColor: themeColors.white, borderRadius: themeRadius.sm, padding: themeSpace.md, marginBottom: themeSpace.md, border: `1px solid ${themeColors.warning50}`,
             }}
             >
               <h5 style={{
             ...
             >
               ...
               <p style={{ fontSize: '11px', color: themeColors.gray600, margin: 0 }}>
                 {availableTroopLeaders.length}
                 {' '}
                 troop leaders available
       </p>
             </div>
               ...
               {availableTroopLeaders
                 .filter(
                   (leader: any) => leader.name?.toLowerCase().includes(troopLeaderSearchTerm.toLowerCase())
 ...
 || leader.email?.toLowerCase().includes(troopLeaderSearchTerm.toLowerCase()),
         ...
         Cancel
               ...
               >
                 <div>
                 <h5 style={{
                 margin: 0, color: themeColors.text, fontWeight: '600', fontSize: '13px',
               }}
               >
                 {leader.name}
               </h5>
